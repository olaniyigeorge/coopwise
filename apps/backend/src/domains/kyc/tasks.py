...
def process_identity_submission(self, *, kyc_id, document_type, document_number,
    ...
    if not acquire_idempotency_lock(lock_key):
        logger.warning(f"[process_identity_submission] skipped kyc={kyc_id} — lock '{lock_key}' already held")
        return
    ...
    async def _run():
        async with build_kyc_service() as service:   # <-- this line is what was missing
            await service.complete_identity_submission(UUID(kyc_id), data)
