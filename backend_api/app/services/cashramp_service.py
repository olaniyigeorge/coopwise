...
CASHRAMP_URL = (
    ...
    if config.ENV == "dev"
    else "https://staging.api.useaccrue.com/cashramp/api/graphql"
)  # TODO Flip this to use staging in dev. CashRamp not providing staging API KEYS
...
class CashRampService:
    ...
    async def create_customer(
        self, email: str, first_name: str, last_name: str, country_id: str
    ) -> CustomerResponse:
        query = gql(
            """
    ...
    async def initiate_deposit(
        self, ramp_quote_id: str, reference: Optional[str] = None
    ) -> InitiateDepositResponse:
        query = gql(
            """
        ...
        variables = {"rampQuote": ramp_quote_id, "reference": reference}
        async with self.client as session:
            result = await session.execute(query, variable_values=variables)
        return InitiateDepositResponse(**result["initiateRampQuoteDeposit"])

    async def verify_deposit(
        self, payment_request_id: str, proof: Optional[str] = None
    ...
    async def mark_deposit_as_paid(
        self, payment_request_id: str, receipt_url: Optional[str] = None
    ) -> Dict[str, Any]:
        query = gql(
            """
        ...
        variables = {"paymentRequest": payment_request_id, "receipt": receipt_url}
        async with self.client as session:
            result = await session.execute(query, variable_values=variables)
