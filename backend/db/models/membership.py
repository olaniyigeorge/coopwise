...
class GroupMembership(Base):
    ...
    status = Column(
        Enum(MembershipStatus), default=MembershipStatus.clicked, nullable=False
    )
    joined_at = Column(DateTime, default=None, nullable=True)

    payout_position = Column(
    ...
    user = relationship(
        ...
        overlaps="cooperatives,group",
    )
    inviter = relationship(
        "User", back_populates="invited_memberships", foreign_keys=[invited_by]
    )

    group = relationship(
        "CooperativeGroup", back_populates="memberships", overlaps="users,cooperatives"
    )
